...
class SeismicRecordDownSampling(nn.Module):
    ...
    def forward(self, x):
        ...
        width = x.shape[3] # width: 70  x:{Tensor: (5,1000,70)}
        new_size = [width * 8, width] # {list: 2} [560, 70]
        dimred0 = F.interpolate(x, size=new_size, mode='bilinear', align_corners=False) # {Tensor: (2,7,560,70)}

        dimred1 = self.pre_dim_reducer1(dimred0)  # {Tensor: (2,8,280,70)}
        dimred2 = self.pre_dim_reducer2(dimred1)  # {Tensor: (2,7,280,70)}
        dimred3 = self.pre_dim_reducer3(dimred2)  # {Tensor: (2,16,140,70)}
        dimred4 = self.pre_dim_reducer4(dimred3)  # {Tensor: (2,16,140,70)}
        dimred5 = self.pre_dim_reducer5(dimred4)  # {Tensor: (2,32,70,70)}
        dimred6 = self.pre_dim_reducer6(dimred5)  # {Tensor: (2,32,70,70)}
...
class LossDDNet:
    def __init__(self, weights=[1, 1], entropy_weight=[1, 1]):
        ...
        ew = torch.from_numpy(np.array(entropy_weight).astype(np.float32)).cpu()
        self.criterion2 = nn.CrossEntropyLoss(weight = ew).to(device)    # For multi-classification, the current issue is a binary problem (either black or white).
...
if __name__ == '__main__':
    ...
    model = DDNet70Model(n_classes=1, in_channels=5, is_deconv=True, is_batchnorm=True)
